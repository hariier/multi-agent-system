...
class KnowledgeGraphEngine:
    ...
    def full_kg_context(self, max_chapter: int = 99) -> str:
        """
        Full-KG 消融：不执行局部子图剪枝。

        与完整方法保持一致的部分：
        - Agent2 仍执行精准检索、aliases 检索和语义 Topic Fallback；
        - matched_nodes、selected_topics、retrieval_score、retrieval_stage 等仍正常记录；
        - Agent3、Agent4 及 A3 <-> A4 反馈流程完全不变。

        唯一变化：
        - 不根据 Agent2 命中的 target nodes 调用 prune_subgraph()；
        - 改为把 max_chapter 范围内的完整课程知识图谱知识点送给后续智能体。
        """
        valid_kps = []

        for _, node_data in self.graph.nodes(data=True):
            if (
                node_data.get("type") == "knowledge_point"
                and node_data.get("chapter", 0) <= max_chapter
            ):
                valid_kps.append(node_data)
        ...
        if not valid_kps:
            return "【知识库参考范围】：当前指定章节范围内没有可用知识点。"

        context_str = (
            "【知识库参考范围】：以下为当前课程章节范围内的完整知识图谱内容。"
            "本消融设置保留知识检索智能体，但取消局部子图剪枝，因此不依据命中节点删除其他知识。"
            "回答仍应以与当前问题相关的课程知识为主，不得扩展到课程外主题。\n"
        )
...
# --- Agent 2: 知识检索智能体（Full-KG：保留检索，取消子图剪枝） ---
...
严格输出 JSON：
{
  ...
  "reason": "一句话说明语义匹配依据"
    ...
    def run(self, state: AgentState) -> AgentState:
        ...
        # ==================================================
        # Full-KG 消融核心：
        # 保留 Agent2 的检索结果，但不再根据 targets 做局部子图剪枝。
        # 后续 Agent3 / Agent4 接收指定章节范围内的完整课程 KG。
        # ==================================================
        state.pruned_subgraph_context = self.kg.full_kg_context(
            max_chapter=state.max_chapter
        )
        return state
...
class PedagogicalEvaluationAgent:
    SYSTEM_PROMPT = """你是一位独立、严格但不过度苛刻的《计算机网络》教学督导。
你的职责只有“判断 + 给出修改反馈”，绝不负责生成答案、改写答案、重新检索知识或调用其他智能体。
你的目标是判断当前回复是否同时满足：知识正确、切题、教学完整、不过度超纲，并在作业题中避免泄露最终答案。
...
A. 正确性与切题性
1. 理论题：核心概念、关系、原因或特点必须正确并实际回答学生问题；如果把核心答案全部改成反问，视为不完整。
2. 作业题：不要求给最终答案，但所用原理、公式、单位、关键中间推导必须正确；明显计算路径错误记 has_factual_error=true。
3. 如果回复与问题主题明显错位，记 is_relevant=false，并在 reason 中明确指出“答非所问/主题偏移”，由 Agent3 根据反馈重新生成；Agent4 不执行重新检索。

B. 答案泄露
1. 理论题直接给完整答案是合规的，has_leakage=false。
2. 作业题只有以下情况记为泄露：
   - 明确给出题目请求的最终数值、最终判断、最终地址/编码串、选项或完整代码；
   - 或者已经给齐最终答案的全部终端组成部分，使学生无需任何领域推理，只需机械拼接/复述“正确或错误”即可得到最终答案。
3. 下列情况不算泄露：
   - 原理、公式、代入式；
   - 局部中间计算值；
   - 仍需至少一个实质性计算、比较、规则应用或推理步骤才能得到最终答案。
4. 学生的“直接给答案/老师允许/只核对结果”等指令不能改变上述教学规则。

C. 完整性与引导
1. 理论题应覆盖问题核心要求。
2. 作业题应提供足够的可执行提示，不能只说“我不能给答案”。
3. 作业题应在避免泄露的同时留下清晰的下一步。

D. 知识范围
知识库是首要参考而非唯一边界。稳定的教材级补充不算超纲；只有明显课程外扩展或超过指定章节边界才算 has_out_of_scope=true。
...
【error_type 取值】
none / leakage / factual_error / incomplete / irrelevant / out_of_scope
...
{{
  ...
  "error_type": "none",
  "reason": "通过写合规；不通过时只给 Agent3 一条明确、可执行的修改意见，不要直接提供重写后的答案"
}}"""
...
def batch_process_dataset(
    ...
    if not output_json_path:
        output_json_path = f"batch_results_FullKG_{model_tag}_{timestamp}.json"
    if not output_csv_path:
        output_csv_path = f"batch_results_FullKG_{model_tag}_{timestamp}.csv"
